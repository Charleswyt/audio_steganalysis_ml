            ...
            if strcmp(stego_method{s}, 'HCM')
                for e = 1:length(embedding_rate)
                    ...
                    if exist(feature_stego_path, 'file')
                        ...
                        try
                            result = training(feature_cover, feature_stego, percent, 'model.mat', 'True');
                ...
                for w = 1:length(width)
                    for e = 1:length(embedding_rate)
                        if embedding_rate(e) == 10
                            feature_stego_name = [stego_method{s}, '_W_', num2str(width(w)), '_B_', num2str(bitrate(b)), '_ER_', num2str(embedding_rate(e))];
                        else
                            feature_stego_name = [stego_method{s}, '_W_', num2str(width(w)), '_B_', num2str(bitrate(b)), '_ER_0', num2str(embedding_rate(e))];
                        end
                        feature_stego_path = fullfile(feature_stego_mat_dir, stego_method{s}, feature_type{f}, [feature_stego_name, '.mat']);
                        if exist(feature_stego_path, 'file')
                            feature_stego_file = load(feature_stego_path);
