...
stego_dir = 'E:\Myself\2.database\mtap\txt\stego\EECS';
    ...
    for w = 1:length(width)
        for e = 1:length(embedding_rates)
            if embedding_rates(e) == 10
                stego_file_name = ['EECS_W_', num2str(width(w)), '_B_', num2str(bitrates(b)), '_ER_' num2str(embedding_rates(e))];
            else
                stego_file_name = ['EECS_W_', num2str(width(w)), '_B_', num2str(bitrates(b)), '_ER_0' num2str(embedding_rates(e))];
            end
            ...
            if exist(stego_files_path, 'file')
                ...
                fprintf('===========================\n');
                fprintf('type: %s\n', stego_file_name);
                fprintf('FPR: %d, FNR: %.2f, ACC: %.2f\n', 100*results.FPR, 100*results.FNR, 100*results.ACC);
            else
                continue;
            end
        end
